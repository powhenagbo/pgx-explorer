...
<body>
  ...
  <div class="drawing-container">
    <div class="canvas-container">
      <svg id="output-svg" viewbox="0 0 500 500" width="500" height="500" xmlns="http://www.w3.org/2000/svg"></svg>
    </div>
  </div>
  ...
  <script>
    ...
    function draw() {
      ...
      SmilesDrawer.parse(input.value, function (tree) {
        let renderedSvg = document.getElementById('output-svg');
        ...
        svgDrawer.draw(tree, svg, 'dark', false);
        svgDrawer.svgWrapper.toCanvas('output-canvas')
        let td = performance.now() - t;
        log.innerHTML = '&nbsp;';
        log.style.visibility = 'hidden';

        new Noty({
          text: 'Drawing took ' + td.toFixed(3) + 'ms with a total overlap score of ' + svgDrawer.getTotalOverlapScore().toFixed(3) + '.',
          killer: true,
          timeout: 2000,
          animation: {
            open: null,
        ...
        console.log(svgDrawer.getMolecularFormula());
      }, function (err) {
        log.innerHTML = err;
